...
filepath = "checkpoints_padchest/beto_indio_0001.h5"
...
for i in tqdm(range(len(val_comment))):
    encoding = tokenizer.encode_plus(val_comment[i]  , max_length = 128 , pad_to_max_length = True , truncation = "longest_first" )
    input_ids , attention_id = encoding["input_ids"] , encoding["attention_mask"]
...
y_val = new_val.drop(["Report"] , axis=1)
val_id = np.array(padded_ids_val)
val_mask = np.array(mask_ids_val)


validation_data = ([val_id , val_mask], y_val)
...
#model = TFXLMForSequenceClassification.from_pretrained('xlm-mlm-100-1280', from_pt=True)
...
auroc = MultipleClassAUROC(class_names = ['Pulmon', 'Calcificacion', 'Cuerpos extranos', 'Mediastino e hilios pulmonares', 'Pleura y diafragma y pared abdominal', 'Patologica', 'Unchanged'], sequence = validation_data, weights_path = filepath)
...
model.fit(x = [train_id , train_mask] , y = y_train,
          ...
          epochs=4, callbacks = [checkpoint, auroc,], class_weight = class_weights
)
